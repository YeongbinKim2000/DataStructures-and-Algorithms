...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        ...
        if (size == backingArray.length) {
            ...
            backingArray = (T[]) new Object[this.backingArray.length * 2];
            for (int i = 1; i < size + 1; i++) {
                backingArray[i] = tempArr[mod(front, tempArr.length)];
                front++;
            }
            front = 0;
        } else {
            front = mod((front - 1 + backingArray.length), backingArray.length);
        }
    ...
    public T removeLast() {
        ...
        if (size == 1) {
            ...
            int idx = mod((front + size - 1), backingArray.length);
            removedData = backingArray[idx];
            backingArray[idx] = null;
        }
        size--;
